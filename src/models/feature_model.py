"""PaDiM-lite anomaly model using pretrained CNN features + Mahalanobis distance."""
...
class FeatureModelState:
    """Serializable metadata for the feature model."""
...
class _FeatureExtractor(torch.nn.Module):
    """Extract intermediate features from a frozen backbone."""
...
class CategoryFeatureModel(BaseAnomalyModel):
    """Per-category PaDiM-lite: Gaussian patch embeddings + Mahalanobis distance.

    Key optimizations over naive implementation:
    - Random dimension reduction (D_full -> 100) before covariance, following PaDiM
    - Fully vectorized covariance and Mahalanobis computation (no Python loops)
    """
    ...
    def fit(self, images: List[np.ndarray]) -> "CategoryFeatureModel":
        ...
        # Step 2: Random dimension reduction
        ...
        # Step 3: Vectorized per-patch mean
        self.mean = all_emb_reduced.mean(axis=0)  # (D, N)

        # Step 4: Vectorized per-patch covariance inverse
        # centered: (num_images, D, N)
        centered = all_emb_reduced - self.mean[np.newaxis, :, :]
        # For each patch j: cov_j = (1/(n-1)) * centered[:,:,j].T @ centered[:,:,j]
        # Batched: centered transposed to (N, num_images, D) then batch matmul
        centered_t = centered.transpose(2, 0, 1)  # (N, num_images, D)
        # cov_batch: (N, D, D) = (N, D, n_images) @ (N, n_images, D)
        ...
        # Step 5: Compute thresholds from training anomaly maps
        train_scores = []
        all_pixels = []
        for img in images:
    ...
    def _compute_anomaly_map(self, image: np.ndarray) -> np.ndarray:
        """Compute Mahalanobis-distance anomaly map (fully vectorized)."""
        embedding = self._extract_embedding(image)
        embedding = self._reduce_dims(embedding)  # (D, N)
        diff = embedding - self.mean  # (D, N)

        # Vectorized Mahalanobis: for each patch j, d_j = sqrt(diff[:,j] @ cov_inv[j] @ diff[:,j])
        # diff transposed: (N, D)
        diff_t = diff.T  # (N, D)
        # Batch: (N, 1, D) @ (N, D, D) @ (N, D, 1) -> (N, 1, 1)
